...
/**
 * @brief CMD6、3、16命令处理回调函数,传参由框架自动完成
 * @param pucRegBuffer: 数据载体
 * @param usAddress: 寄存器地址
 * @param usNRegs: 要操作的寄存器的个数
 * @param eMode: 操作模式
 */
...
/**
 * @brief FreeModbus轮询监听任务
 */
void mb_task(void *pvParameters)
{
    eMBInit(MB_RTU, 0x01, 1, 9600, MB_PAR_NONE); /* RTU方式,从机地址1,使用串口1,波特率9600,无校验 */
    eMBEnable();
    while (1)
    {
        eMBPoll();
